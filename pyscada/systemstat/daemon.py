...
from pyscada.systemstat import client
...
def run():
    label   = 'pyscada.systemstat.daemon'
    pid     = str(os.getpid())
    # read the global settings
    if settings.PYSCADA_SYSTEMSTAT.has_key('polling_interval'):
        dt_set = float(settings.PYSCADA_SYSTEMSTAT['polling_interval'])
    else:
    ...
    log.notice("started systemstat dataaquisition daemon")
    err_count = 1
    # main loop
