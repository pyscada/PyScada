...
from pyscada.models import Variable, Device, Unit, RecordedDataFloat, RecordedDataInt, RecordedDataBoolean, RecordedTime, BackgroundTask, RecordedData
    ...
    for var_idx in range(0,active_vars.count(),10):
        ...
        for var in var_slice:
            ...
            if data.has_key(var.pk):
                ...
                # write dummy data
                bf.write_data(var.name,_cast_value([0]*len(timevalues),value_class),\
                id = var.pk,\
                description=var.description,\
                value_class = validate_value_class(value_class),\
                unit = udunit)
