...
class Handler(GenericDevice):
    ...
    def write_data(self,variable_id, value):
        ...
        if j == 0:
            logger.error("MDO3014-Inst NOT connected")
            return None
        if variable_id == 'init_BODE':
            #Reset and init Osc
            Vepp = RecordedData.objects.last_element(variable_id=11)
#            self.inst.write('*CLS')
            self.inst.write('*RST;:SEL:CH1 1;:SEL:CH2 1;:HORIZONTAL:POSITION 0;:CH1:YUN "V";:CH1:SCALE '+str(1.2*float(Vepp.value())/(2*4))+';:CH2:YUN "V";:CH2:BANdwidth 10000000;:CH1:BANdwidth 10000000;:TRIG:A:TYP EDGE;:TRIG:A:EDGE:COUPLING DC;:TRIG:A:EDGE:SOU CH1;:TRIG:A:EDGE:SLO FALL;:TRIG:A:MODE NORM')
            return True
        elif variable_id == 'find_phase':
            i=0
            F = RecordedData.objects.last_element(variable_id=15)
            while F is None and i<10:
                F = RecordedData.objects.last_element(variable_id=15)
                i += 1
            Vsff = value
            Vsmax = float(Vsff)*float(math.sqrt(2))
#            logger.info("F : %s" %F.value())
#            logger.info("T/div : %s" %round(float(float(4)/(float(10)*float(F.value()))),6))
            CMD = str(':HORIZONTAL:SCALE '+str(round(float(float(4)/(float(10)*float(F.value()))),6))+';:CH2:SCALE '+str(1.4*float(Vsmax)/4)+';:TRIG:A:LEV:CH1 '+str(0.8*float(Vsmax))+
                ';:MEASUrement:IMMed:SOUrce1 CH1;:MEASUrement:IMMed:SOUrce2 CH2;:MEASUREMENT:IMMED:TYPE PHASE')
            self.inst.write(CMD)
            time.sleep(20/F.value())
#            time.sleep(5)
#            CMD = str(':MEASUrement:IMMed:SOUrce1 CH1;:MEASUrement:IMMed:SOUrce2 CH2;:MEASUREMENT:IMMED:TYPE PHASE;:MEASUREMENT:IMMED:VALUE?')
            CMD = str(':MEASUREMENT:IMMED:VALUE?')
            Phase = self.inst.query(CMD)
            cwt = DeviceWriteTask(variable_id=9, value=1, start=time.time())
            cwt.save()
#            self.write_data("find_gain", 1)
            return self.parse_value(Phase)
            ...
            N = RecordedData.objects.last_element(variable_id=16)
            N_new = N.value() + 1
            F = RecordedData.objects.last_element(variable_id=15)
            Fmin = RecordedData.objects.last_element(variable_id=12)
            Fmax = RecordedData.objects.last_element(variable_id=13)
            nb_points = RecordedData.objects.last_element(variable_id=14)
            #Variation de F en log
            F_new = Fmin.value() + (Fmax.value() - Fmin.value())*log10(float(9*(N_new - 1)+nb_points.value() - 1)/float(nb_points.value() - 1))
            F_new = int(F_new)
            #Variation F linéaire
            #F_new = F.value() + (Fmax.value() - Fmin.value())/nb_points.value()
            if F_new > Fmax.value():
                logger.info("BODE finished")
            else:
                cwt = DeviceWriteTask(variable_id=15, value=F_new, start=time.time())
                cwt.save()
                cwt = DeviceWriteTask(variable_id=16, value=N_new, start=time.time())
                cwt.save()
                cwt = DeviceWriteTask(variable_id=7, value=F_new, start=time.time())
                cwt.save()
