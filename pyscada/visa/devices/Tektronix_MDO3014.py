...
class Handler(GenericDevice):
    ...
    def write_data(self,variable_id, value):
        ...
        if j == 0:
            logger.error("MDO3014-Instrument non connecté")
            return None
...
#            logger.info("N_new : %s - k : %s - F : %s" %(N_new, k, F_new))
            ...
            if F_new > Fmax.value():
                ...
                cwt = DeviceWriteTask(variable_id=Variable.objects.get(name='BODE_F').id, value=F_new, start=time.time())
                cwt.save()
                cwt = DeviceWriteTask(variable_id=Variable.objects.get(name='BODE_n').id, value=N_new, start=time.time())
                cwt.save()
                cwt = DeviceWriteTask(variable_id=Variable.objects.get(name='Set_Freq_GBF').id, value=F_new, start=time.time())
                cwt.save()
