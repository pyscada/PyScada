    ...
    if type(instance) is VariableProperty:
        if instance.id is not None:  # existing object
            ...
            if str(instance.last_value) != str(previous.value()):
                logger.debug(
                    "VP %s new value = %s - %s"
                    % (str(instance), instance.last_value, previous.value())
                )
...
def _reinit_daq_daemons(sender, instance, **kwargs):
    ...
    if type(instance) is Device:
        try:
            ...
            try:
                bp = BackgroundProcess.objects.get(
                    ...
                    failed=False,
                    label="pyscada."
                    + str(instance.protocol.protocol)
                    + "-"
                    + str(instance.id),
                )
        ...
        try:
            ...
            try:
                bp = BackgroundProcess.objects.get(
                    ...
                    failed=False,
                    label="pyscada."
                    + str(instance.device.protocol.protocol)
                    + "-"
                    + str(instance.device_id),
                )
            except BackgroundProcess.DoesNotExist:
        ...
        for bp_pk in list(
            instance.variable_set.all().values_list("device__protocol_id").distinct()
        ):
            try:
            ...
            try:
                ...
                try:
                    if device.protocol_id == 1:
                        return False
                    bp = BackgroundProcess.objects.get(pk=device.protocol_id)
                except:
                    return False
            bp.restart()
                ...
                try:
                    ...
                    try:
                        bp = BackgroundProcess.objects.get(
                            ...
                            failed=False,
                            label="pyscada."
                            + str(instance.variable.device.protocol.protocol)
                            + "-"
                            + str(instance.variable.device_id),
                        )
                    except BackgroundProcess.DoesNotExist:
                    ...
                    try:
                        bp = BackgroundProcess.objects.get(
                            pk=instance.variable.device.protocol_id
                        )
                    except:
                        return False
                bp.restart()
